...
def ocr_from_image(image_path=None, image_url=None):
    if image_url:
        ...
        if image is None:
            print("Error: Image not loaded. Check the URL:", image_url)
            return ""
    else:
...
def ocr_from_image(image_path=None, image_url=None):
    ...
    detected_text = " ".join([text[1] for text in result])
    print("Detected Text:", detected_text)
    return detected_textimport time
import numpy as np
import pytesseract
import my_hand_mouse_utils
import easyocr
...
def decide_drag_direction(hand_points, min_drag_distance_left, min_drag_distance_right, last_point):
    ...
    dist = manhattan_distance(last_point, current_point)  # Use Manhattan distance
...
def decide_drag_direction(hand_points, min_drag_distance_left, min_drag_distance_right, last_point):
    ...
    dist = euclidean_distance(last_point, current_point)  # Use Euclidean distance
...
def fill_right_hand_points(image, one_hand_landmarks, image_width, image_height, screen_width, hand_points):
